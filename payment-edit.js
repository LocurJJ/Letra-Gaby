(function () {
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      const draft = readStockDraft();
      const validation = validateStockDraft(draft);
      ...
      state.orders.push(buildStockOverrideOrder(draft, validation));
      saveState(state);
  ...
  function validateStockDraft(draft) {
  ...
  function buildStockOverrideOrder(draft, validation) {
    return {
